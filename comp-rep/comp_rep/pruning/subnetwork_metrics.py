...
from comp_rep.pruning.subnetwork_set_operations import intersection, union


def intersection_over_union(
    subnetwork_A: Transformer, subnetwork_B: Transformer
) -> float:
    intersection_model = intersection(subnetwork_A, subnetwork_B)
    union_model = union(subnetwork_A, subnetwork_B)
    intersection_sum = 0.0
    union_sum = 0.0
    for m_A, m_B in zip(intersection_model.modules(), union_model.modules()):
